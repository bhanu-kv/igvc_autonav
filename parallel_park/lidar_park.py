...
class MinimalPublisher(Node):
    ...
    def __init__(self):
        ...
        self.cone1_subscription = self.create_subscription(Pose, '/cone1_pose', self.cone_callback, 10)
        self.cone2_subscription = self.create_subscription(Pose, '/cone2_pose', self.cone_callback, 10)
            ...
            if not self.first_message_received:
                ...
                if len(fin_coords) > 0:
                    ...
                    for cluster_id in unique_clusters:
                        if cluster_id != -1:
                            # Use NumPy for boolean indexing instead of list
                            cluster_points = fin_coords[clusters == cluster_id]
                            
                            # Define the function to calculate the radius from the center (xc, yc)
                            def calc_radius(xc, yc, x, y):
                                return np.sqrt((x - xc)**2 + (y - yc)**2)

                            # Define the function to calculate the residuals (difference between each point's radius and the mean radius)
                            def residuals_circle(params, x, y):
                                xc, yc = params
                                ri = calc_radius(xc, yc, x, y)
                                return ri - np.mean(ri)

                            # Get the x and y coordinates from the cluster points
                            x = cluster_points[:, 0]
                            y = cluster_points[:, 1]

                            # Initial guess for the center (mean of points)
                            x0 = np.mean(x)
                            y0 = np.mean(y)
                            ...
                            # Calculate the radius based on the estimated center
    ...
    def cone_callback(self, msg):
        self.cones.append(np.array([msg.position.x, msg.position.y]))

        if not self.first_message_received and len(self.cones) % 2 == 0 and self.cones and self.odom_receieved:
            self.new_centroids = {}
            for i in self.cones:
                x, y = i[0], i[1]
                self.new_centroids[x**2 + y**2] = i
            ...
            self.get_logger().info(f"Cone 1: {self.cones[0]}")
            self.get_logger().info(f"Cone 2: {self.cones[1]}")
    ...
    def publish_trajectory(self, mean_centroid,centroids,final_centroid):
        ...
        for t in np.arange(3.14,2*th ,-0.1):
        ...
        for t in np.arange(-3.14+2*th,0,0.1):
